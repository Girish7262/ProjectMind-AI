package com.acciobuild.knowledge.security.filter;
...
/**
 * Request filter parsing JWT Bearer headers, validating tokens, mapping
 * UserDetails, and establishing multi-tenant context boundaries.
 */
@Component
        ...
        try {
            if (jwtUtils.validateToken(jwt)) {
                ...
                if (email != null && userIdStr != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                    ...
                    KnowledgeUserDetails principal = new KnowledgeUserDetails(userId, email, organizationId, authorities);
            ...
            log.error("Failed to map security contexts from token: {}", e.getMessage());
